...
//-----------------------------------------------------
REGISTER_OPERATION(CreateChartWithColor)

CreateChartWithColor::CreateChartWithColor() {

}
CreateChartWithColor::CreateChartWithColor(quint64 metaid, const Ilwis::OperationExpression &expr) : CreateChart(metaid, expr) {
}

Ilwis::OperationImplementation *CreateChartWithColor::create(quint64 metaid, const Ilwis::OperationExpression& expr) {
	return new CreateChartWithColor(metaid, expr);
}

bool CreateChartWithColor::execute(ExecutionContext *ctx, SymbolTable& symTable) {
	if (_prepState == sNOTPREPARED)
		if ((_prepState = prepare(ctx, symTable)) != sPREPARED)
			return false;
	...
	QVariantMap extra;
	extra["color"] = _color;
	quint32 modelid = _chartmodel->createChart(_name, _table, _chartType, _xaxis, _yaxis, _zaxis, extra);
...
Ilwis::OperationImplementation::State CreateChartWithColor::prepare(ExecutionContext *ctx, const SymbolTable &symTable) {
	if (_prepState == sNOTPREPARED)
		if ((_prepState = CreateChart::prepare(ctx, symTable)) != sPREPARED)
			return sNOTPREPARED;

	_color = _expression.input<QString>(7);

	return sPREPARED;

}

quint64 CreateChartWithColor::createMetadata() {
	OperationResource operation({ "ilwis://operations/createchartwithcolor" });
	operation.setSyntax("createchartwithcolor(modelid, table, charttype=[Line|Spline|Area|Bar|Pie|Points|Polar|3DLine|3DSpline|3Dbar], xaxis,yaxis,zaxis,colorcolumn)");
	operation.setDescription(TR("creates a chart of the specified type"));
	...
	operation.addInParameter(5, itSTRING, TR("Name Y axis"), TR("Name of the column used for the yaxis"));
	operation.addInParameter(6, itSTRING, TR("Name Z axis"), TR("Name of the column used for the yaxis. Undefined in case we have 2 dim graph"));
	operation.addInParameter(7, itSTRING, TR("Name Color column"), TR("Name of the column for color information"));
	operation.setOutParameterCount({ 0 });
	operation.setKeywords("table,visualization,chart");
