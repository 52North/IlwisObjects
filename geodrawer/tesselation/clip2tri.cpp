...
static const F32 CLIPPER_SCALE_FACT = 10000000.0f;
static const F32 CLIPPER_SCALE_FACT_INVERSE = 0.0000001f;
...
Path clip2tri::removeDuplicates(const Path &inputPolygon) {
	std::map<double, int> all;
	for (int i = 0; i < inputPolygon.size();++i) {
		auto &point = inputPolygon[i];
		//Szudzik's function for uniquely pairing two integers
		double v = point.X >= point.Y ? (point.X * point.Y + point.X + point.Y ): point.X + point.Y * point.Y;
		all[v] = i;
	}
	if (all.size() == inputPolygon.size())
		return inputPolygon;

	std::vector<bool> used(inputPolygon.size(), false);
	for (auto& item : all) {
		used[item.second] = true;
	}
	Path result(all.size());
	int j = 0;
	for (int i = 0; i < inputPolygon.size(); ++i) {
		if ( used[i])
			result[j++] = inputPolygon[i];
	}
	return result;
...
Path clip2tri::upscaleClipperPoints(const vector<Point> &inputPolygon)
{
   ...
   for (S32 i = 0; i < inputPolygon.size(); i++) {
	   IntPoint pnt1 = IntPoint(S64(inputPolygon[i].x * CLIPPER_SCALE_FACT), S64(inputPolygon[i].y * CLIPPER_SCALE_FACT));
	   outputPolygon[i] = pnt1;
   }
   auto cleanedPolygons = removeDuplicates(outputPolygon);
